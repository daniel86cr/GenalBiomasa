...
@Theme("Genal")
@UIScope
@SpringView(name = VistaListadoControlPt.NAME)
public class VistaListadoControlPt extends CustomComponent implements View ,Button.ClickListener {
    /** El nombre de la vista.*/
    public static final String                                                     NAME                   = "vLControlPt";
    /** Necesario para mostrar los controles de PT*/
    private BeanContainer<String, TControlProductoTerminadoVista>                  bcControlPt;
    /** Necesario para mostrar los controles de PT*/
    private BeanContainer<String, TLineaControlProductoTerminadoVista>             bcLineasControlPt;
    ...
    /** El boton para eliminar un control de PT.*/
    private Button                                                                 eliminarButton;
    /** El boton para eliminar una línea de control de PT.*/
    private Button                                                                 eliminar2Button;
    ...
    /** El ID de la línea del control de producto terminado seleccionado.*/
    private String                                                                 idSeleccionado2;
    /** El log de la aplicación.*/
    private static final org.slf4j.Logger                                          log                    = org.slf4j.LoggerFactory.getLogger(VistaListadoControlPt.class);
    ...
    /** Tabla para mostrar los controles de producto terminado del sistema. */
    private Table                                                                  tablaLineaControlPt    = null;
    ...
    public void enter(ViewChangeEvent event) {
        ...
        if (time != null) {
            ...
            try {
                ...
                bcLineasControlPt = new BeanContainer<>(TLineaControlProductoTerminadoVista.class);
                bcLineasControlPt.setBeanIdProperty("id");
                ...
                crearTablaLineaControlPt(permisos);
                ...
                List<TControlProductoTerminadoVista> lRegistros = Utils.generarListaGenerica();

                if (empleado.getIdRol().equals(RolesEnum.OPERARIO_CONTROL_PT.getValue())) {
                    lRegistros = contrVista.obtenerControlesPtCreadosVista(user, f1.getValue(), f2.getValue(), user, time);
                } else {
                    lRegistros = contrVista.obtenerControlesPtVista(f1.getValue(), f2.getValue(), user, time);
                }

                bcControlPt.addAll(lRegistros);
                ...
                Label tituloTablaTotales = new Label("Productos");
                tituloTablaTotales.setStyleName("tituloTamano18");

                viewLayout.addComponent(tituloTablaTotales);
                if (empleado.getIdRol().equals(RolesEnum.MASTER.getValue()) || empleado.getIdRol().equals(RolesEnum.ADMINISTRADOR.getValue())) {
                    viewLayout.addComponent(eliminar2Button);
                }
                viewLayout.addComponent(tablaLineaControlPt);
    ...
    private void crearBotones() {
        ...
        eliminarButton = new Button("Eliminar Control PT", this);
        eliminar2Button = new Button("Eliminar línea Control PT", this);
    ...
    private void eventosBotones() {
        ...
        modificarButton.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                ...
                if (idSeleccionado != null && !idSeleccionado.contains(",")) {
                    ...
                    getUI().getNavigator().navigateTo(VistaControlPt.NAME + "/" + idSeleccionado);
        ...
        eliminarButton.addClickListener(new ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
                ...
                if (idSeleccionado != null && !idSeleccionado.isEmpty()) {
                    ...
                    MessageBox.createQuestion().withCaption(appName).withMessage("¿Quieres eliminar el/los registro(s) seleccionado(s)? Esta acción no se puede deshacer").withNoButton()
                              .withYesButton(() -> eliminarControlPt(), ButtonOption.caption("Sí")).open();
        ...
        eliminar2Button.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                if (idSeleccionado2 != null && !idSeleccionado2.isEmpty()) {

                    MessageBox.createQuestion().withCaption(appName).withMessage("¿Quieres eliminar la línea del control de PT seleccionada? Esta acción no se puede deshacer").withNoButton()
                              .withYesButton(() -> eliminarLineaControlPt(), ButtonOption.caption("Sí")).open();
    ...
    private void crearTablaControlPt(TPermisos permisos) {
        tablaControlPt = new TablaGenerica(new Object[] { "id", "fecha", "indNave1", "indNave2", "indNave3", "linea", "indBio", "idCliente", "numeroPedido", "usuCrea", "fechaCrea", "observaciones" }, new String[] { "Nº Control PT", "Fecha control PT", "¿Nave 1?", "¿Nave 2?", "¿Nave 3?", "Línea", "¿BIO?", "Cliente", "Nº Pedido", "Registrado por", "Fecha creación", "Observaciones" }, bcControlPt);
        ...
        */
        tablaControlPt.addItemClickListener(new ItemClickListener() {
            ...
            public void itemClick(ItemClickEvent event) {
                ...
                if (!idSeleccionado.contains(",") && !idSeleccionado.isEmpty()) {
                    bcLineasControlPt.removeAllContainerFilters();
                    bcLineasControlPt.removeAllItems();
                    idSeleccionado2 = null;
                    try {
                        bcLineasControlPt.addAll(contrVista.obtenerLineasControlProductoTerminadoVista(Integer.valueOf(idSeleccionado), user, time));
                    } catch (GenasoftException tbe) {
                        log.error("La sesión es inválida, se ha iniciado sesión en otro dispositivo.");
                        // Si no se encuentran permisos con el rol especificado, informamos al empleado y cerramos sesión.                
     ...
     * @return El Grid con las columnas.
     */
    private void crearTablaLineaControlPt(TPermisos permisos) {
        tablaLineaControlPt = new TablaGenerica(new Object[] { "id", "idProdudcto", "numCajasPedido", "numCajasReal", "idVariedad", "idCalibre", "idDiametro", "pesoCaja", "indFotoCaja", "indFotoPale", "indFotoConfeccion", "indFotoEtiqueta", "indContaminaFisica", "indContaminaQuimica", "indContaminaBiologica", "usuCrea", "fechaCrea", "observaciones" }, new String[] { "#", "Producto", "Nº Cajas pedido", "Nº Cajas real", "Variedad", "Calibre", "Diámetro", "Peso caja/confección teórico", "Fotos caja", "Fotos palet", "Fotos etiq. Ext", "Fotos etiq. Int", "Contaminación física", "Contaminación química", "Contaminación biológica", "Registrado por", "Fecha creación", "Observaciones" }, bcLineasControlPt);
        tablaLineaControlPt.addStyleName("big striped");
        tablaLineaControlPt.setPageLength(20);
        tablaLineaControlPt.setId("1");

        tablaLineaControlPt.addItemClickListener(new ItemClickListener() {

            @Override
            public void itemClick(ItemClickEvent event) {
                idSeleccionado2 = (String) event.getItemId();

                try {
                    // Cargamos los datos de los pesajes, en función de si es caja, calibre o confección.
                    @SuppressWarnings("unchecked")
                    BeanItem<TControlProductoTerminadoVista> aux = (BeanItem<TControlProductoTerminadoVista>) tablaControlPt.getItem(idSeleccionado2);
                    ...
                    if (cPt.getIndFlowPack().equals("1") || cPt.getIndFlowPack().equals("2") || cPt.getIndFlowPack().equals("3") || cPt.getIdProdudcto().contains("MANGO")) {
                        vTablaPesConf.setVisible(true);
                        bcPesajesConfeccion.removeAllItems();
                        bcPesajesConfeccion.addAll(contrVista.obtenerPesajesConfeccionIdLineaControlPtVista(Integer.valueOf(idSeleccionado2), user, time));
                        vTablaPesCaja.setVisible(false);
                        vTablaPesCalibre.setVisible(true);
                        bcPesajesCalibres.removeAllItems();
                        bcPesajesCalibres.addAll(contrVista.obtenerPesajesCalibreIdLineaControlPtVista(Integer.valueOf(idSeleccionado2), user, time));
                    } else {
                        vTablaPesCaja.setVisible(true);
                        vTablaPesCalibre.setVisible(true);
                        vTablaPesConf.setVisible(false);
                        bcPesajesCajas.removeAllItems();
                        bcPesajesCajas.addAll(contrVista.obtenerPesajesCajaIdLineaVista(Integer.valueOf(idSeleccionado2), user, time));
                    ...
                    bcPesajesPenetromia.removeAllItems();
                    bcPesajesPenetromia.addAll(contrVista.obtenerPesajesPenetromiaIdLineaVista(Integer.valueOf(idSeleccionado2), user, time));

                    bcPesajesBrix.removeAllItems();
                    bcPesajesBrix.addAll(contrVista.obtenerPesajesBrixIdLineaVista(Integer.valueOf(idSeleccionado2), user, time));
        ...
        tablaLineaControlPt.setMultiSelect(false);

        tablaLineaControlPt.setCellStyleGenerator(new Table.CellStyleGenerator() {

            @SuppressWarnings("unchecked")
            @Override
            public String getStyle(Table source, Object itemId, Object propertyId) {
                BeanItem<TLineaControlProductoTerminadoVista> aux = (BeanItem<TLineaControlProductoTerminadoVista>) source.getItem(itemId);

                TLineaControlProductoTerminadoVista cPt = aux.getBean();
                if (cPt.getEstado().equals("Incompleto")) {
                    return "red";
                } else {
                    return "green";
    ...
    private void aplicarFiltro() {
        ...
        if (!fClientes.getValue().equals("Todos")) {
            // Set new filter for the "Name" column
            filter = new FiltroContainer("idCliente", (String) ".*(" + fClientes.getValue().toString().toLowerCase() + ").*", status);
            bcControlPt.addContainerFilter(filter);
            entra = true;
        }
    ...
    private void cambiarFechas() {
        ...
        try {
            List<TControlProductoTerminadoVista> lRegistros;
            if (empleado.getIdRol().equals(RolesEnum.OPERARIO_CONTROL_PT.getValue())) {
                lRegistros = contrVista.obtenerControlesPtCreadosVista(user, f1.getValue(), f2.getValue(), user, time);
            } else {
                lRegistros = contrVista.obtenerControlesPtVista(f1.getValue(), f2.getValue(), user, time);
    ...
    /**
     * Método que nos realiza la desactivación del control de producto terminado seleccionado(s).
     * @param idSeleccionado El Id del/los control de PT seleccionado(s) (o ids separados por ',') a desactivar.
     */
    private void eliminarControlPt() {

        try {

            if (!idSeleccionado.contains(",")) {
                idSeleccionado = idSeleccionado.concat(",");
            }
            String[] ids = idSeleccionado.split(",");
            int i = 0;
            String result = "";
            while (i < ids.length) {
                result = contrVista.eliminarControlPt(Integer.valueOf(ids[i]), user, time);

                if (result.equals(Constants.OPERACION_OK)) {
                    bcControlPt.removeItem(ids[i]);
                }
                i++;
            }
     ...
     * @param idSeleccionado El Id del/los control de PT seleccionado(s) (o ids separados por ',') a desactivar.
     */
    private void eliminarLineaControlPt() {
