...
@Theme("Genal")
@UIScope
@SpringView(name = VistaTrazabilidades.NAME)
public class VistaTrazabilidades extends CustomComponent implements View ,Button.ClickListener {
    /** El nombre de la vista.*/
    public static final String                        NAME                     = "vTrazabilidades";
    ...
    /** El boton para guardar los cambios introducidos.*/
    private Button                                    incluirLineaVentaButton;
    /** El boton para guardar los cambios introducidos.*/
    private Button                                    guardarLineaVentaButton;
    /** El boton para guardar los cambios introducidos.*/
    ...
    private static final org.slf4j.Logger             log                      = org.slf4j.LoggerFactory.getLogger(VistaTrazabilidades.class);
    ...
    /** Tabla para mostrar las ventas del sistema. */
    private Table                                     tablaNuevaVenta          = null;
    /** Tabla para mostrar los pedidos del sistema. */
    ...
    /** Para filtrar por basura.  */
    private ComboBox                                  cbCodsPallet;
    /** Para filtrar por basura.  */
    private ComboBox                                  cbLotesPallet;
    ...
    private VerticalLayout                            cabeceraPantalla2;
    ...
    /** Lista con las partidas identificadas en las compras. */
    private List<String>                              lCodsPallet;
    /** Lista con las partidas identificadas en las compras. */
    private List<String>                              lLotesPallet;
    ...
    /** Lista de códigos de pallet por los que está filtrando. */
    private ListSelect                                lsCodPales;
    /** Lista de lotes de pallet por los que está filtrando. */
    private ListSelect                                lsLotesPales;
    /** Lista de calidades por los que está filtrando. */
    ...
    public void enter(ViewChangeEvent event) {
        ...
        if (time != null) {
            ...
            try {
                ...
                Label texto = new Label("Trazabilidades");
                ...
                //mostrarColumnasTablaComprasCambios(lColumnas);
                ...
                // lColumnas = contrVista.obtenerCamposPantallaTablaEmpleado(user, NAME, Integer.valueOf(tablaVentasCambios.getId()), user, time);

                //mostrarColumnasTablaVentasCambios(lColumnas);

                mCompras = contrVista.obtenerComprasEjercicioActivo(user, time);
                mVentas = contrVista.obtenerVentasEjercicioActivo(user, time);

                //mCompras = new TreeMap<Date, List<TComprasVista>>();
                //mVentas = new TreeMap<Date, List<TVentasVista>>();
                ...
                vNuevasTablas.addComponent(tablaNuevaVenta);
                ...
                lCodsPallet = contrVista.obtenerCodsPallets(user, time);
                lLotesPallet = contrVista.obtenerLotesPallets(user, time);
                ...
                viewLayout.addComponent(cabeceraPantalla2);

                //viewLayout.addComponent(filtro);

                Label lbl3 = new Label(" ");
                lbl3.setHeight(30, Sizeable.Unit.EM);
                ...
                cabeceraPantalla2.addComponent(cabeceraPantalla);
                cabeceraPantalla2.addComponent(filtro);
                cabeceraPantalla2.addComponent(tituloTablaColores);
                cabeceraPantalla2.addComponent(tablaColores);
                ...
                tVentas.addComponent(eliminarLineaVentaButton);
                ...
                btns.addComponent(incluirLineaVentaButton);
                btns.addComponent(guardarLineaVentaButton);
                tLineas.addComponent(btns);
                ...
                cabeceraPantalla2.addComponent(horBot);
                cabeceraPantalla2.addStyleName("posLayoutFixed");
                ...
                tablaNuevaVenta.setVisible(false);
                ...
                //     cal.setTime(Utils.obtenerPrimerDiaMesEnCurso());

                //     cal.add(Calendar.MONTH, -2);

                //     fechaDesdeCompra.setValue(cal.getTime());
                //     fechaHastaCompra.setValue(Utils.obtenerUltimoDiaMesEnCurso());
    ...
    private void crearBotones() {
        ...
        incluirLineaVentaButton = new Button("Añadir línea de trazabilidad", this);
        guardarLineaVentaButton = new Button("Guardar cambios trazabilidad", this);
    ...
    public void buttonClick(ClickEvent event) {
        ...
        if (event.getButton().equals(guardarColumnasComprasButton)) {
        ...
        } else if (event.getButton().equals(incluirVentaButton)) {
            if (!tablaNuevaVenta.isVisible()) {
                tablaNuevaVenta.setVisible(true);
                guardarVentaButton.setVisible(true);
                borrarVentaButton.setVisible(true);
            }
            TVentasVista aux = new TVentasVista();
            aux.setId("" + tablaNuevaVenta.getItemIds().size() + 1);
            bcNuevaVenta.addBean(aux);
            tablaNuevaVenta.setPageLength(tablaNuevaCompra.getItemIds().size());
        } else if (event.getButton().equals(incluirLineaVentaButton)) {
            TLineasVentasVista aux = new TLineasVentasVista();
            aux.setId("-" + tablaNuevaCompra.getItemIds().size() + 1);
            bcLineasVentas.addBean(aux);
            tablaNuevaCompra.setPageLength(tablaNuevaCompra.getItemIds().size());
        } else if (event.getButton().equals(guardarLineaVentaButton)) {
            guardarLineaTrazabilidad();
        } else if (event.getButton().equals(guardarCompraButton)) {
            guardarLineaCompra();
        } else if (event.getButton().equals(guardarVentaButton)) {
            guardarLineaVenta();
            ...
            if (idSeleccionadoNVenta == null || idSeleccionadoNVenta.isEmpty()) {
                ...
                tablaNuevaVenta.removeItem(idSeleccionadoNVenta);
            }
            ...
            if (idSeleccionadoVentas == null || idSeleccionadoVentas.isEmpty()) {
                ...
                String[] ids;
                if (idSeleccionadoVentas.contains(",")) {
                    ids = idSeleccionadoVentas.split(",");
                } else {
                    ids = new String[1];
                    ids[0] = new String();
                    ids[0] = idSeleccionadoVentas;
                }
                ...
                int cnt = 0;
                for (cnt = 0; cnt < ids.length; cnt++) {
                    bRes = (BeanItem<TVentasVista>) tablaVentas.getItem(ids[cnt]);
                    res = bRes.getBean();

                    mVentasEliminadas.put(res.getId(), res);
                    bcVentas.removeItem(res.getId());
                }
            ...
            if (mComprasEliminadas.isEmpty() && mVentasEliminadas.isEmpty() && lIdsComprasModificadas.isEmpty() && lIdsVentasModificadas.isEmpty() && !indPedido && !indAlbaran && !indCalibre && !indTraza && !indProv && !indKgs && !indVaried
                ...
                cbTotalLotesGuacamole.setValue(8);
    ...
    private void crearTablaCompras(TPermisos permisos) {
        tablaCompras = new TablaGenerica(new Object[] { "nombreDescriptivo", "albaranFin", "fechaFin", "partidaFin", "cajasFin", "kgsBrutoFin", "loteFin", "pesoNetoFin", "kgsDisponibles", "cerrada", "productoFin", "variedadFin", "proveedorFin", "origenFin", "familiaFin", "ggnFin", "calidadFin", "calibreFin" }, new String[] { "Descripción importación", "Albarán", "Fecha", "Partida", "Nº de cajas", "Peso bruto", "Trazabilidad", "Peso neto", "Kgs disponibles", "¿Cerrada?", "Producto", "Variedad", "Entidad", "Orígen", "Plantilla de producto", "Global Gap", "Certificación", "Calibre" }, bcCompras);
    ...
    private void crearTablaComprasCambios(TPermisos permisos) {
        tablaComprasCambios = new TablaGenerica(new Object[] { "albaranFin", "fechaFin", "partidaFin", "pesoNeto", "pesoNetoFin", "kgsDisponibles", "cerrada", "cajasFin", "kgsBruto", "kgsBrutoFin", "loteFin", "variedadFin", "proveedorFin", "origenFin", "familiaFin", "ggn", "ggnFin", "calidadFin", "calibreFin" }, new String[] { "Albarán(fin)", "Fecha(fin)", "Partida(fin)", "Peso neto(ini)", "Peso neto(fin)", "Kgs disponibles", "¿Cerrada?", "Nº de cajas(fin)", "Peso bruto(ini)", "Peso bruto(fin)", "Trazabilidad(fin)", "Variedad(fin)", "Entidad(fin)", "Orígen(fin)", "Plantilla de producto(fin)", "Global Gap(ini)", "Global Gap(fin)", "Calidad(fin)", "Calibre(fin)" }, bcCompras);
        ...
        tablaComprasCambios.setColumnWidth("fechaFin", 77);
    ...
    private void crearTablaVentas(TPermisos permisos) {
        tablaVentas = new TablaGenerica(new Object[] { "nombreDescriptivo", "pedidoFin", "lineaPedido", "albaranFin", "calibreFin", "idPaleFin", "numBultosFin", "numBultosPaleFin", "clienteFin", "fechaVentaFin", "kgsFin", "kgsNetosFin", "variedadFin", "origenFin", "calidadVentaFin", "confeccionFin", "familiaFin" }, new String[] { "Descripción importación", "Pedido de venta", "Línea pedido", "Albarán de venta", "Calibre", "ID de palé", "Bultos mov. venta", "Bultos por palé", "Cliente", "Fecha salida", "Kilos", "Peso neto teo. venta", "Variedad", "Orígen venta", "Certificación venta", "Confección", "Producto" }, bcVentas);
                ...
                if (event.isDoubleClick()) {
                    ...
                    if (event.getPropertyId() != null) {
                        if (event.getPropertyId().equals("albaranFin")) {
                            ...
                            comprobarFiltrosAplicados();
                        } else if (event.getPropertyId().equals("idPaleFin")) {
                            List<String> lCalidades = (List<String>) lsCodPales.getItemIds();

                            Boolean entra = false;
                            // Comprobamos si el albarán por el que quiere hacer el filtro, está ya en la lista de albaranes de compra.
                            for (String fam : lCalidades) {
                                if (fam.equals(res.getIdPaleFin())) {
                                    entra = true;
                                    break;
                                }
                            }
                            if (!entra) {
                                lsCodPales.addItem(res.getIdPaleFin());
                            } else {
                                lsCodPales.removeItem(res.getIdPaleFin());
                            }
                            comprobarFiltrosAplicados();
                        } else if (event.getPropertyId().equals("loteMovAlmFi")) {
                            List<String> lCalidades = (List<String>) lsLotesPales.getItemIds();

                            Boolean entra = false;
                            // Comprobamos si el albarán por el que quiere hacer el filtro, está ya en la lista de albaranes de compra.
                            for (String fam : lCalidades) {
                                if (fam.equals(res.getLoteMovAlmFi())) {
                                    entra = true;
                                    break;
                                }
                            }
                            if (!entra) {
                                lsLotesPales.addItem(res.getLoteMovAlmFi());
                            } else {
                                lsLotesPales.removeItem(res.getLoteMovAlmFi());
                            }
                            comprobarFiltrosAplicados();
                        }
                    }
    ...
    private void crearTablaLineasVentas() {
        tablaLineasVentas = new TablaGenerica(new Object[] { "certificacionFin", "ggn", "kgsTrazadosFin", "loteFin", "referenciaCompraFin", "proveedorFin", "albaranCompraFin", "indBasura" }, new String[] { "Certificación", "GGN", "Kgs", "Lote", "Referencia compra", "Proveedor", "Albarán compra", "¿Basura?" }, bcLineasVentas);
        ...
        tablaLineasVentas.setCellStyleGenerator(new Table.CellStyleGenerator() {
            ...
            public String getStyle(Table source, Object itemId, Object propertyId) {
                ...
                if (Integer.valueOf(venta.getId()) > 0) {
    ...
    private void crearTablaVentasCambios(TPermisos permisos) {
        tablaVentasCambios = new TablaGenerica(new Object[] { "pedidoFin", "albaranFin", "lineaPedido", "calibre", "calibreFin", "idPaleFin", "loteMovAlmFi", "clienteFin", "fechaVentaFin", "kgsNetos", "kgsNetosFin", "kgsEnvaseFin", "variedad", "variedadFin", "origen", "origenFin", "calidadVenta", "calidadVentaFin", "confeccionFin", "familia", "familiaFin" }, new String[] { "Pedido de venta(fin)", "Albarán de venta(fin)", "Línea pedido", "Calibre(ini)", "Calibre(fin)", "Cód. Pallet", "Trazabilidad Pallet", "Cliente(fin)", "Fecha(fin)", "Kilos(ini)", "Kilos(fin)", "Kilos envase", "Variedad(ini)", "Variedad(fin)", "Orígen venta(ini)", "Orígen venta(fin)", "Certificación (ini)", "Certificación (fin)", "Confección(fin)", "Producto(ini)", "Producto(fin)" }, bcVentas);
        ...
        tablaVentasCambios.setTableFieldFactory(new TableFieldFactory() {
            ...
            public Field createField(Container container, Object itemId, Object propertyId, Component uiContext) {
                ...
                // If you want to disable edition on a column, use ReadOnly
                if ((!propertyId.toString().endsWith("Fin") && !propertyId.toString().endsWith("Fi")) || propertyId.toString().contains("client") || propertyId.toString().contains("fech") || propertyId.toString().contains("confeccionFin")) {
                    if (propertyId.toString().equals("lineaPedido")) {
                        return field;
                    } else {
                        field.setReadOnly(true);
                        return field;
    ...
    private void crearTablaVentasTotales() {
        tablaVentasTotales = new TablaGenerica(new Object[] { "kgsNetos", "kgsNetosFin", }, new String[] { "Kilos totales", "Kilos trazados" }, bcVentasTotales);
    ...
    private void crearTablaNuevaVenta() {
        tablaNuevaVenta = new TablaGenerica(new Object[] { "nombreDescriptivo", "pedido", "albaran", "calibre", "lote", "idPale", "numBultos", "numBultosPale", "proveedor", "cliente", "fechaVenta", "kgs", "kgsNetos", "variedad", "origen", "calidadVenta", "confeccion", "familia" }, new String[] { "Descripción importación", "Pedido de venta", "Albarán de venta", "Calibre", "Trazabilidad", "ID de palé", "Bultos mov. venta", "Bultos por palé", "Productor", "Entidad", "Fecha salida", "Kilos", "Peso neto teo. venta", "Variedad", "Orígen venta", "Certificación venta", "Confección", "Producto" }, bcNuevaVenta);
        tablaNuevaVenta.addStyleName("big striped");
        tablaNuevaVenta.setEditable(true);
        tablaNuevaVenta.setPageLength(1);
        tablaNuevaVenta.addItemClickListener(new ItemClickListener() {
            @Override
            public void itemClick(ItemClickEvent event) {
                idSeleccionadoNVenta = (String) event.getItemId();
            }
        });
    }
    ...
    private void crearComponenteFiltro() {
        ...
        cabeceraPantalla2 = new VerticalLayout();
        cabeceraPantalla.setSpacing(true);
        cabeceraPantalla.setMargin(true);
        ...
        fTipoFiltro.addItem("Código pallet");
        ...
        fTipoFiltro.addItem("Trazabilidad pallet");
        ...
        fTipoFiltro.addValueChangeListener(new ValueChangeListener() {
            ...
            public void valueChange(ValueChangeEvent event) {
                ...
                if (fTipoFiltro.getValue() != null) {
                    ...
                    } else if (fTipoFiltro.getValue().equals("Código pallet")) {
                        cbCodsPallet.setVisible(true);
                        lsCodPales.setVisible(true);
                    } else if (fTipoFiltro.getValue().equals("Trazabilidad pallet")) {
                        cbLotesPallet.setVisible(true);
                        lsLotesPales.setVisible(true);
                    }
                }
            }
        });
        ...
        cabeceraPantalla.setVisible(true);
        //cabeceraPantalla.addStyleName("posLayoutFixed");
        ...
        // Combobox con los lote
        cbLotesPallet = new ComboBox();
        cbLotesPallet.addItems(lLotesPallet);
        cbLotesPallet.setFilteringMode(FilteringMode.CONTAINS);
        cbLotesPallet.addValueChangeListener(new ValueChangeListener() {

            @SuppressWarnings("unchecked")
            @Override
            public void valueChange(ValueChangeEvent event) {
                if (cbLotesPallet.getValue() != null) {
                    List<String> lAlbar = (List<String>) lsLotesPales.getItemIds();
                    Boolean entra = false;
                    for (String al : lAlbar) {
                        if (al.equals((String) cbLotesPallet.getValue())) {
                            entra = true;
                            break;
                        }
                    }
                    ...
                    if (!entra) {
                        lsLotesPales.addItem(cbLotesPallet.getValue());
                    }
                    comprobarFiltrosAplicados();
                    cbLotesPallet.clear();
                    fTipoFiltro.clear();
        ...
        // Lista de lote que se están filtrando
        lsLotesPales = new ListSelect();
        lsLotesPales.addValueChangeListener(new ValueChangeListener() {

            @Override
            public void valueChange(ValueChangeEvent event) {
                if (lsLotesPales.getValue() != null) {
                    lsLotesPales.removeItem(lsLotesPales.getValue());
                }
                comprobarFiltrosAplicados();
            }
        });
        ...
        // Combobox con los lote
        cbCodsPallet = new ComboBox();
        cbCodsPallet.addItems(lCodsPallet);
        cbCodsPallet.setFilteringMode(FilteringMode.CONTAINS);
        cbCodsPallet.addValueChangeListener(new ValueChangeListener() {

            @SuppressWarnings("unchecked")
            @Override
            public void valueChange(ValueChangeEvent event) {
                if (cbCodsPallet.getValue() != null) {
                    List<String> lAlbar = (List<String>) lsCodPales.getItemIds();
                    Boolean entra = false;
                    for (String al : lAlbar) {
                        if (al.equals((String) cbCodsPallet.getValue())) {
                            entra = true;
                            break;
                        }
                    }
                    ...
                    if (!entra) {
                        lsCodPales.addItem(cbCodsPallet.getValue());
                    }
                    comprobarFiltrosAplicados();
                    cbCodsPallet.clear();
                    fTipoFiltro.clear();
        ...
        // Lista de lote que se están filtrando
        lsCodPales = new ListSelect();
        lsCodPales.addValueChangeListener(new ValueChangeListener() {

            @Override
            public void valueChange(ValueChangeEvent event) {
                if (lsCodPales.getValue() != null) {
                    lsCodPales.removeItem(lsCodPales.getValue());
                }
                comprobarFiltrosAplicados();
            }
        });
        ...
        cal.setTime(Utils.obtenerPrimerDiaYearEnCurso());
        cal.setTime(Utils.obtenerUltimoDiaYearEnCurso());
        ...
        hComps.addComponent(cbLotesPallet);
        hComps.addComponent(lsLotesPales);
        hComps.addComponent(cbCodsPallet);
        hComps.addComponent(lsCodPales);
    ...
    private void aplicarFiltroVentas(Boolean cargarCompras) {
        ...
        if (((List<String>) lsCodPales.getItemIds()).size() != 0) {
            List<String> lCods = (List<String>) lsCodPales.getItemIds();
            String cods = "";
            for (String cod : lCods) {
                cods = cods + cod + " ";
            }
            // Set new filter for the "Name" column
            String[] params = cods.split(" ");
            String filtro = "";
            for (int i = 0; i < params.length; i++) {
                filtro = filtro + params[i] + "|";
            }
            filtro = filtro.toLowerCase();
            filter = new FiltroContainer("idPaleFin", filtro.toLowerCase(), status);
            bcVentas.addContainerFilter(filter);
        }

        if (((List<String>) lsLotesPales.getItemIds()).size() != 0) {
            List<String> lLotes = (List<String>) lsLotesPales.getItemIds();
            String lotes = "";
            for (String lote : lLotes) {
                lotes = lotes + lote + " ";
            }
            ...
            filtro = filtro.toLowerCase();
            filter = new FiltroContainer("loteMovAlmFi", filtro.toLowerCase(), status);
            bcVentas.addContainerFilter(filter);
        }
                        ...
                        if (lsClientes.getItemIds().size() == 0 && lsProveedoresVentas.getItemIds().size() == 0) {
                            ...
                            if (lsFamiliasVentas.getItemIds().size() == 0) {
                                ...
                                if (lsLoteVentas.getItemIds().size() == 0) {
                                    // Código pallet
                                    if (lsCodPales.getItemIds().size() == 0) {
                                        // Lote pallet
                                        if (lsLotesPales.getItemIds().size() == 0) {
                                            // Paises
     ...
     */
    @SuppressWarnings("unchecked")
    private void guardarLineaVenta() {
        List<String> lIdsNuevos = bcNuevaVenta.getItemIds();
        try {
            ...
            List<String> lResults = Utils.generarListaGenerica();
            for (String id : lIdsNuevos) {
                tablaNuevaVenta.getItem(id);
                BeanItem<TVentasVista> bRes = (BeanItem<TVentasVista>) tablaNuevaVenta.getItem(id);
                TVentasVista res = bRes.getBean();

                // Comprobamos que se hayan introducido correctamente los datos fechas, números, etc.
                if (res.getFechaVenta() == null) {
                    res.setFechaVenta(Utils.generarFecha());
                }

                if (res.getNumBultos() != null && !res.getNumBultos().isEmpty()) {
                    // Comprobamos que los números sean números
                    try {
                        Integer.valueOf(res.getNumBultos());
                    } catch (NumberFormatException e) {
                        Notification aviso = new Notification("El campo 'Bultos mov. venta' de la línea " + cnt + ", no tiene el valor correcto, el valor debe ser numérico sin decimales", Notification.Type.ERROR_MESSAGE);
                        aviso.setPosition(Position.MIDDLE_CENTER);
                        aviso.show(Page.getCurrent());
                        break;
                    }
                }
                if (res.getNumBultosPale() != null && !res.getNumBultosPale().isEmpty()) {
                    // Comprobamos que los números sean números
                    try {
                        Integer.valueOf(res.getNumBultosPale());
                    } catch (NumberFormatException e) {
                        Notification aviso = new Notification("El campo 'Bultos por palé' de la línea " + cnt + ", no tiene el valor correcto, el valor debe ser numérico sin decimales", Notification.Type.ERROR_MESSAGE);
                        aviso.setPosition(Position.MIDDLE_CENTER);
                        aviso.show(Page.getCurrent());
                        break;
                    }
                ...
                if (res.getNombreDescriptivo() == null || res.getNombreDescriptivo().isEmpty()) {
                    res.setNombreDescriptivo("Creación línea venta manual. Fecha: " + new SimpleDateFormat("dd/MM/yyyy").format(Utils.generarFecha()));
                }

                res.setUsuCrea("" + user);
                res.setFechaCrea(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss").format(Utils.generarFecha()));
                String result = contrVista.guardarLineaVenta(res, false, user, time);
    ...
    /**
     * Método que nos guarda en el sistema una nueva línea de trazabilidad a manija.
     * Esto nos guarda "TODOS" los datos que se están mostrando en la tabla de trazabilidades.
     */
    @SuppressWarnings("unchecked")
    private void guardarLineaTrazabilidad() {
        List<String> lIdsLineasVentas = bcLineasVentas.getItemIds();
        try {

            Integer cnt = 1;

            List<String> lResults = Utils.generarListaGenerica();
            for (String id : lIdsLineasVentas) {
                tablaLineasVentas.getItem(id);
                BeanItem<TLineasVentasVista> bRes = (BeanItem<TLineasVentasVista>) tablaLineasVentas.getItem(id);
                TLineasVentasVista res = bRes.getBean();
                res.setIdVenta(idSeleccionadoVentas);

                if (res.getKgsTrazadosFin() == null || res.getKgsTrazadosFin().equals(Double.valueOf(0))) {
                    Notification aviso = new Notification("Se debe informar los kilos de la trazabilidad", Notification.Type.ERROR_MESSAGE);
                    aviso.setPosition(Position.MIDDLE_CENTER);
                    aviso.show(Page.getCurrent());
                    break;
                }

                if (res.getCertificacionFin() == null || res.getCertificacionFin().trim().isEmpty()) {
                    Notification aviso = new Notification("Se debe informar la certificación", Notification.Type.ERROR_MESSAGE);
                    aviso.setPosition(Position.MIDDLE_CENTER);
                    aviso.show(Page.getCurrent());
                    break;
                }

                if (res.getLoteFin() == null || res.getLoteFin().trim().isEmpty()) {
                    Notification aviso = new Notification("Se debe informar el campo lote", Notification.Type.ERROR_MESSAGE);
                    aviso.setPosition(Position.MIDDLE_CENTER);
                    aviso.show(Page.getCurrent());
                    break;
                }

                if (res.getAlbaranCompraFin() == null || res.getAlbaranCompraFin().trim().isEmpty()) {
                    Notification aviso = new Notification("Se debe informar el albarán de compra", Notification.Type.ERROR_MESSAGE);
                    aviso.setPosition(Position.MIDDLE_CENTER);
                    aviso.show(Page.getCurrent());
                    break;
                }

                String result = contrVista.guardarLineaTrazabilidad(res, user, time);
            ...
            if (lResults.size() > 1) {
                ...
                if (lResults.get(0).equals(Constants.OPERACION_OK)) {
                    ...
                    Notification aviso = new Notification(contrVista.obtenerDescripcionCodigo(lResults.get(0)), Notification.Type.ASSISTIVE_NOTIFICATION);
                    aviso.setPosition(Position.MIDDLE_CENTER);
                    aviso.show(Page.getCurrent());
                }
            }
    ...
    private void mostrarCorrectos() {
        ...
        filter = new FiltroContainer("cerrada", "sí", status);
        bcVentas.addContainerFilter(filter);
        calcularTotales();
    ...
    private void mostrarErrores() {
        ...
        filter = new FiltroContainer("cerrada", "no", status);
        bcVentas.addContainerFilter(filter);
        calcularTotales();
    ...
    private void accionCorregir(Integer numLotesMax, Integer numLotesGuacaMax, String opcion) {
        ...
        try {
            ...
            //mVentasErroneas = contrVista.corregirVentasErroneas(user, time);
            ...
            if (mVentasErroneas.isEmpty()) {
                ...
                Notification aviso = new Notification("Ha habido un total de " + mVentasErroneas.size() + " errores que no se han podido solucionar", Notification.Type.HUMANIZED_MESSAGE);
                aviso.setPosition(Position.MIDDLE_CENTER);
                aviso.show(Page.getCurrent());
                contrVista.enviarNotificacion("[GENASOFT] CORRECCION ERRORES", "Ha habido un total de " + mVentasErroneas.size() + " errores que no se han podido solucionar");
            }
    ...
    private void quitarTodosFiltros() {
        ...
        // Lotes pallets
        lsLotesPales.removeAllItems();

        // Códigos pallets.
        lsCodPales.removeAllItems();
    ...
    private void comprobarFiltrosAplicados() {
        ...
        if (lsLotesPales.getItemIds().size() != 0) {
            filtr = filtr.concat("- Lote pallet: ");
            lFiltros = (List<String>) lsLotesPales.getItemIds();
            for (String filtro : lFiltros) {
                filtr = filtr.concat(filtro).concat(", ");
            }
        }
        ...
        if (lsCodPales.getItemIds().size() != 0) {
            filtr = filtr.concat("- Código pallet: ");
            lFiltros = (List<String>) lsCodPales.getItemIds();
            for (String filtro : lFiltros) {
                filtr = filtr.concat(filtro).concat(", ");
            }
        }
    ...
    private void ocultarComponentesFiltros() {
        ...
        cbCodsPallet.setVisible(false);
        lsCodPales.setVisible(false);
        cbLotesPallet.setVisible(false);
        lsLotesPales.setVisible(false);
    }
