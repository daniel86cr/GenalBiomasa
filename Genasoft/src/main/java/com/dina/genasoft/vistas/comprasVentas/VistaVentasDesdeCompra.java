...
@Theme("Genal")
@UIScope
@SpringView(name = VistaVentasDesdeCompra.NAME)
public class VistaVentasDesdeCompra extends CustomComponent implements View ,Button.ClickListener {
    /** El nombre de la vista.*/
    public static final String                   NAME                = "vNVentasCompra";
    ...
    /** El boton para guardar los cambios introducidos.*/
    private Button                               comprasVentasButton;
    /** El boton para imprimir la hoja de ruta.*/
    ...
    private static final org.slf4j.Logger        log                 = org.slf4j.LoggerFactory.getLogger(VistaVentasDesdeCompra.class);
    ...
    public void enter(ViewChangeEvent event) {
        ...
        if (time != null) {
            ...
            try {
                ...
                Label texto = new Label("Generar ventas desde compra");
                ...
                if (!Utils.booleanFromInteger(permisos.getCrearVentasPedidoCompra())) {
                ...
                horBot.addComponent(comprasVentasButton);
    ...
    private void crearBotones() {
        ...
        guardarButton = new Button("Generar ventas", this);
        ...
        comprasVentasButton = new Button("Listado de compras/ventas", this);
    ...
    public void buttonClick(ClickEvent event) {
        ...
        if (event.getButton().equals(guardarButton)) {
            if (bcCompras.getItemIds().isEmpty()) {
                Notification aviso = new Notification("Para generar las ventas, es necesario cargar la compra", Notification.Type.WARNING_MESSAGE);
                aviso.setPosition(Position.MIDDLE_CENTER);
                aviso.show(Page.getCurrent());
            } else {
                DateField date = new DateField("Fecha compra: ");
                Label lbl = new Label();
                lbl.setValue("Si no se informan los campos, se cogerá los valores de la(s) compra(s).");

                ComboBox cCalidad = new ComboBox("Calidad compra");
                cCalidad.addItem("BIO");
                cCalidad.addItem("CONVENCIONAL");
                cCalidad.setFilteringMode(FilteringMode.CONTAINS);

                VerticalLayout hor = new VerticalLayout();
                hor.setSpacing(true);
                hor.addComponent(lbl);
                hor.setComponentAlignment(lbl, Alignment.TOP_CENTER);
                hor.addComponent(date);
                hor.setComponentAlignment(date, Alignment.TOP_CENTER);
                hor.addComponent(cCalidad);
                hor.setComponentAlignment(cCalidad, Alignment.TOP_CENTER);

                MessageBox.createQuestion().withCaption(appName).withMessage(hor).withNoButton().withYesButton(() ->

                generarVentas(cCalidad.getValue(), date.getValue()), ButtonOption.caption("Generar")).open();
            ...
            if (tablaVentas.isVisible()) {
                if ((!tablaVentas.getItemIds().isEmpty()) && idSeleccionadoVentas != null && !idSeleccionadoVentas.isEmpty()) {
                    try {
                        guardarRegistrosTablasExportacion();
            ...
            try {
                ...
                List<TComprasVista> lCompras = contrVista.obtenerComprasAlbaranesLotesPartidas(lAlbaranes, lLotes, lPartidas, user, time);
        ...
        } else if (event.getButton().equals(comprasVentasButton)) {
            getUI().getNavigator().navigateTo(VistaComprasVentas.NAME);
        }
    }
    ...
    private void crearTablaCompras(TPermisos permisos) {
        tablaCompras = new TablaGenerica(new Object[] { "albaranFin", "fechaFin", "partidaFin", "cajasFin", "loteFin", "pesoNetoFin", "kgsDisponibles", "variedadFin", "proveedorFin", "origenFin", "familiaFin", "ggnFin", "calidadFin" }, new String[] { "Albarán", "Fecha", "Partida", "Nº de cajas", "Trazabilidad", "Peso neto", "Kgs disponibles", "Variedad", "Entidad", "Orígen", "Producto", "Global Gap", "Certificación" }, bcCompras);
        tablaCompras.addStyleName("big striped");
        tablaCompras.setEditable(false);
        tablaCompras.setMultiSelect(true);
        tablaCompras.setPageLength(20);
    ...
    private void generarVentas(Object obj, Date fechaCompra) {
        try {
            ...
            for (String id : lIds) {
                ...
                idCompra = contrVista.generarVentasDesdeCompra(res, fechaCompra, calidad, user, time);
            }
            ...
            if (idCompra > 0) {
                ...
                // Accedemos al listado de compras/ventas ficticias.
                getUI().getNavigator().navigateTo(VistaComprasVentas.NAME + "/" + idCompra);

            } else {
                Notification aviso = new Notification("No se ha podido realizar la operación, contacta con el administrador", Notification.Type.ERROR_MESSAGE);
                aviso.setPosition(Position.MIDDLE_CENTER);
                aviso.show(Page.getCurrent());
            }
        } catch (GenasoftException tbe) {
