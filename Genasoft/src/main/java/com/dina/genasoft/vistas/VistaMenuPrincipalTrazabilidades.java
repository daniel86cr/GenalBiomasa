...
import com.dina.genasoft.vistas.comprasVentas.VistaVentasDesdeCompra;
...
@Theme("Genal")
@UIScope
@SpringView(name = VistaMenuPrincipalTrazabilidades.NAME)
public class VistaMenuPrincipalTrazabilidades extends CustomComponent implements View ,Button.ClickListener {

    /** El nombre de la vista.*/
    public static final String            NAME = "menuTrazabilidades";
    ...
    private static final org.slf4j.Logger log  = org.slf4j.LoggerFactory.getLogger(VistaMenuPrincipalTrazabilidades.class);
    ...
    /** Botón para mostrar el entorno de compras Vs ventas.*/
    private Button                        bComprasVentas;
    /** Botón para mostrar el entorno de compras Vs ventas.*/
    private Button                        bComprasVentasErrores;
    /** Botón para mostrar el entorno de informes.*/
    private Button                        bLibroTrazabilidades;
    /** Botón para mostrar el entorno de balance de masas. */
    private Button                        bBalanceMasas;
    /** Botón para mostrar el entorno de importación de datos. */
    private Button                        bImportar;
    /** Botón para mostrar comprobar si los kgs de venta supera a los de compra por un lote . */
    private Button                        bKgsMal;
    /** Botón para generar ventas a partir de una partida de compra . */
    private Button                        bCompraVenta;
    ...
    public void enter(ViewChangeEvent event) {
        ...
        if (time != null) {
            try {
                ...
                if (!Utils.booleanFromInteger(permisos.getPantallaMenuPrincipalTrazabilidades())) {
    ...
    private void eventosBotones() {
        ...
        bComprasVentas.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                getUI().getNavigator().navigateTo(VistaTrazabilidades.NAME + "/" + empleado.getId());

            }
        });

        bComprasVentasErrores.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                getUI().getNavigator().navigateTo(VistaErrores.NAME + "/" + empleado.getId());

            }
        });

        bBalanceMasas.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                getUI().getNavigator().navigateTo(VistaBalanceMasas.NAME + "/" + empleado.getId());

            }
        });

        // Evento para acceder a la vista principal de facturación.
        bImportar.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                getUI().getNavigator().navigateTo(VistaImportacion.NAME + "/" + empleado.getId());

            }
        });

        // Evento para crear un nuevo empleado.
        bLibroTrazabilidades.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                getUI().getNavigator().navigateTo(VistaLibroTrazabilidad.NAME + "/" + empleado.getId());

            }
        });

        bKgsMal.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                try {
                    contrVista.comprobarComprasVentas();
                } catch (GenasoftException e) {
                    Notification aviso = new Notification(e.getMessage(), Notification.Type.ERROR_MESSAGE);
                    aviso.setPosition(Position.MIDDLE_CENTER);
                    aviso.show(Page.getCurrent());
                }
        ...
        bCompraVenta.addClickListener(new ClickListener() {

            public void buttonClick(ClickEvent event) {

                getUI().getNavigator().navigateTo(VistaVentasDesdeCompra.NAME + "/" + empleado.getId());
    ...
    private void crearBotones() {
        if (bComprasVentas == null) {
            bComprasVentas = new Button("Trazabilidades importadas", this);
            bComprasVentas.addStyleName("wide tall big");
            bComprasVentas.setWidth(18, Sizeable.Unit.EM);
            bComprasVentas.setHeight(4, Sizeable.Unit.EM);
            bComprasVentasErrores = new Button("Comprobación trazabilidades", this);
            bComprasVentasErrores.addStyleName("wide tall big");
            bComprasVentasErrores.setWidth(18, Sizeable.Unit.EM);
            bComprasVentasErrores.setHeight(4, Sizeable.Unit.EM);
            bLibroTrazabilidades = new Button("Libro de trazabilidad", this);
            bLibroTrazabilidades.addStyleName("wide tall big");
            bLibroTrazabilidades.setWidth(18, Sizeable.Unit.EM);
            bLibroTrazabilidades.setHeight(4, Sizeable.Unit.EM);
            bBalanceMasas = new Button("Balance de masas", this);
            bBalanceMasas.addStyleName("wide tall big");
            bBalanceMasas.setWidth(18, Sizeable.Unit.EM);
            bBalanceMasas.setHeight(4, Sizeable.Unit.EM);
            bImportar = new Button("Importar ficheros", this);
            bImportar.addStyleName("wide tall big");
            bImportar.setWidth(18, Sizeable.Unit.EM);
            bImportar.setHeight(4, Sizeable.Unit.EM);
            bCompraVenta = new Button("Ventas desde compra", this);
            bCompraVenta.addStyleName("wide tall big");
            bCompraVenta.setWidth(18, Sizeable.Unit.EM);
            bCompraVenta.setHeight(4, Sizeable.Unit.EM);
            bKgsMal = new Button("Kgs. Compras Vs. Ventas", this);
            bKgsMal.addStyleName("wide tall big");
            bKgsMal.setWidth(18, Sizeable.Unit.EM);
            bKgsMal.setHeight(4, Sizeable.Unit.EM);
    ...
    private VerticalLayout incluirBotones() {
        Label l1 = new Label(" ");
        l1.setHeight(3, Sizeable.Unit.EM);
        Label l2 = new Label(" ");
        ...
        if (Utils.booleanFromInteger(permisos.getImportar())) {
            botones.addComponent(bImportar);
            botones.setComponentAlignment(bImportar, Alignment.MIDDLE_RIGHT);
            botones.addComponent(l1);
        }
        if (Utils.booleanFromInteger(permisos.getComprasVentas())) {
            botones.addComponent(bComprasVentas);
            botones.setComponentAlignment(bComprasVentas, Alignment.MIDDLE_RIGHT);
            botones.addComponent(l5);
        }
        if (Utils.booleanFromInteger(permisos.getErrores())) {
            botones.addComponent(bComprasVentasErrores);
            botones.setComponentAlignment(bComprasVentasErrores, Alignment.MIDDLE_RIGHT);
            botones.addComponent(l6);
        }
        if (Utils.booleanFromInteger(permisos.getTrazabilidades())) {
            botones.addComponent(bLibroTrazabilidades);
            botones.setComponentAlignment(bLibroTrazabilidades, Alignment.MIDDLE_RIGHT);
            botones.addComponent(l2);
        }
        if (Utils.booleanFromInteger(permisos.getBalanceMasas())) {
            botones.addComponent(bBalanceMasas);
            botones.setComponentAlignment(bBalanceMasas, Alignment.MIDDLE_RIGHT);
            botones.addComponent(l3);
        }
        if (Utils.booleanFromInteger(permisos.getComprasVentasFict())) {
            botones.addComponent(bCompraVenta);
            botones.setComponentAlignment(bCompraVenta, Alignment.MIDDLE_RIGHT);
            botones.addComponent(l4);
        }
        if (Utils.booleanFromInteger(permisos.getKgsMal())) {
            botones.addComponent(bKgsMal);
            botones.setComponentAlignment(bKgsMal, Alignment.MIDDLE_RIGHT);
        }
        return botones;
    }
    ...
    private VerticalLayout imagenes() {
        ...
        Resource resource1 = new ThemeResource("images/img1.png");
        ...
        image1.setWidth(15, Sizeable.Unit.EM);
